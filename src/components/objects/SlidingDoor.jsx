...
const SlidingDoor = ({ width, height, scale }) => {
  // Sliding Door 2D Representation
  // Matches the design of Window.jsx but adapted for door dimensions

  // Native size from Window.jsx: Width 160 (-80 to 80), Height 10 (-5 to 5)
  const nativeWidth = 160;
  const nativeHeight = 10;

  const scaleX = width / nativeWidth;
  // For a door, we might want to keep the "thickness" visual similar to the window, 
  // or scale it. Since it's a 2D plan view, the "height" passed here is actually the wall thickness/depth.
  // Window.jsx uses the passed height to scale Y.
  const scaleY = height / nativeHeight;

  return (
    <g transform={`scale(${scaleX}, ${scaleY})`}>
      {/* Wall part - Transform removed to fill full width */}
      <g>
        <g>
